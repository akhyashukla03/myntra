...
for data in SLIDES_DATA:
    ...
    banner_height = Inches(0.36)
    banner_left = Inches((13.333 - 6.5) / 2)
    banner_top = Inches(0.20)
    ...
    title_box = slide.shapes.add_textbox(Inches(0.7), Inches(0.64), Inches(10.5), Inches(0.85))
    ...
    brand_box = slide.shapes.add_textbox(Inches(11.4), Inches(0.64), Inches(1.3), Inches(0.5))
    ...
    # 3. Slide Content Area Layouts (Full 5.62" vertical height canvas)
    card_top = Inches(1.58)
    card_height = Inches(5.62)
    ...
    if s_num == 1:
        ...
        left_w = Inches(4.3)
        top_h = Inches(4.15)
        add_card_shape(slide, Inches(0.7), card_top, left_w, top_h, COLOR_BG_CARD, COLOR_BORDER)
        box = slide.shapes.add_textbox(Inches(0.82), card_top + Inches(0.1), left_w - Inches(0.24), top_h - Inches(0.2))
        tf = box.text_frame
        tf.word_wrap = True
        ...
        mid_left = Inches(5.15)
        mid_w = Inches(4.65)
        add_card_shape(slide, mid_left, card_top, mid_w, top_h, COLOR_SYNTHESIS_BG, COLOR_SYNTHESIS_BORDER)
        box_mid = slide.shapes.add_textbox(mid_left + Inches(0.12), card_top + Inches(0.1), mid_w - Inches(0.24), top_h - Inches(0.2))
        tf_mid = box_mid.text_frame
        tf_mid.word_wrap = True
        tf_mid.margin_left = Inches(0.06); tf_mid.margin_right = Inches(0.06)
        tf_mid.margin_top = Inches(0.06); tf_mid.margin_bottom = Inches(0.06)
        ...
        for bold_text, reg_text in data["midCard"]["bullets"]:
            ...
            p_st.font.size = Pt(9.2); p_st.line_spacing = 1.15
            p_st.space_before = Pt(2); p_st.space_after = Pt(3)
        ...
        phone_left = Inches(9.95)
        phone_width = Inches(2.68)
        ...
        for m_idx, (m_val, m_lbl) in enumerate(data["macroMetrics"]):
            ...
            m_box = slide.shapes.add_textbox(m_left, macro_top + Inches(0.1), macro_w, macro_h - Inches(0.2))
    ...
    # ==========================================
        ...
        add_card_shape(slide, Inches(0.7), card_top, Inches(4.5), Inches(1.5), COLOR_WHITE, COLOR_BORDER)
        box = slide.shapes.add_textbox(Inches(0.82), card_top + Inches(0.08), Inches(4.26), Inches(1.34))
        tf = box.text_frame; tf.word_wrap = True
        tf.margin_left = Inches(0.06); tf.margin_right = Inches(0.06)
        p = tf.paragraphs[0]
        p.text = "💡 THE CORE HYPOTHESIS"
        p.font.size = Pt(10.5); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(2)
        p_body = tf.add_paragraph()
        p_body.text = data["hypothesisBox"]
        p_body.font.size = Pt(9); p_body.line_spacing = 1.15; p_body.font.color.rgb = COLOR_TEXT_PRIMARY

        # Top Right: Current Discovery Funnel (4 Process Steps)
        flow_left = Inches(5.35)
        flow_w = Inches(7.283)
        add_card_shape(slide, flow_left, card_top, flow_w, Inches(1.5), COLOR_WHITE, COLOR_BORDER)
        box_hdr = slide.shapes.add_textbox(flow_left + Inches(0.12), card_top + Inches(0.06), flow_w - Inches(0.24), Inches(0.3))
        tf_h = box_hdr.text_frame; tf_h.word_wrap = True
        p_h = tf_h.paragraphs[0]
        p_h.text = "🔄 CURRENT DISCOVERY FLOW (THE GRAVEYARD LOOP)"
        p_h.font.size = Pt(10); p_h.font.bold = True; p_h.font.color.rgb = COLOR_PURPLE_PILL

        step_w = Inches(1.68)
        step_gap = Inches(0.1)
        for s_idx, (step_title, step_desc) in enumerate(data["discoveryFunnel"]):
            s_left = flow_left + Inches(0.12) + s_idx * (step_w + step_gap)
            add_card_shape(slide, s_left, card_top + Inches(0.38), step_w, Inches(1.02), COLOR_SYNTHESIS_BG, COLOR_SYNTHESIS_BORDER)
            s_box = slide.shapes.add_textbox(s_left + Inches(0.04), card_top + Inches(0.40), step_w - Inches(0.08), Inches(0.96))
            ...
            p_st.font.size = Pt(8.5); p_st.font.bold = True; p_st.font.color.rgb = COLOR_MYNTRA_PINK; p_st.space_after = Pt(1)
            p_sd = tf_s.add_paragraph()
            p_sd.text = step_desc
            p_sd.font.size = Pt(7.5); p_sd.line_spacing = 1.08; p_sd.font.color.rgb = COLOR_TEXT_PRIMARY

        # Bottom Left: Competitor Whitespace Teardown
        comp_top = card_top + Inches(1.65)
        comp_w = Inches(5.8)
        comp_h = card_height - Inches(1.65)
        add_card_shape(slide, Inches(0.7), comp_top, comp_w, comp_h, COLOR_BG_CARD, COLOR_BORDER)
        box_comp = slide.shapes.add_textbox(Inches(0.82), comp_top + Inches(0.1), comp_w - Inches(0.24), comp_h - Inches(0.2))
        tf_c = box_comp.text_frame; tf_c.word_wrap = True
        tf_c.margin_left = Inches(0.08); tf_c.margin_right = Inches(0.08)
        p = tf_c.paragraphs[0]
        p.text = "🌐 GLOBAL & DOMESTIC COMPETITOR TEARDOWN"
        p.font.size = Pt(10.5); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(4)

        for comp_platform, comp_text in data["competitorTeardown"]:
            p_cp = tf_c.add_paragraph()
            p_cp.font.size = Pt(8.8); p_cp.line_spacing = 1.15
        ...
        for f_idx, (f_title, f_desc) in enumerate(data["frictionCards"]):
            ...
            add_card_shape(slide, Inches(6.65), f_cur_top, f_card_w, f_card_h, COLOR_SYNTHESIS_BG, COLOR_SYNTHESIS_BORDER)
            f_box = slide.shapes.add_textbox(Inches(6.77), f_cur_top + Inches(0.06), f_card_w - Inches(0.24), f_card_h - Inches(0.12))
            ...
            p_ft.text = "⚠️ " + f_title
            p_ft.font.size = Pt(10); p_ft.font.bold = True; p_ft.font.color.rgb = COLOR_MYNTRA_PINK; p_ft.space_after = Pt(2)
            p_fb = tf_f.add_paragraph()
            p_fb.text = f_desc
            p_fb.font.size = Pt(8.8); p_fb.line_spacing = 1.12; p_fb.font.color.rgb = COLOR_TEXT_PRIMARY
    ...
    # ==========================================
        ...
        evol_h = Inches(1.3)
        add_card_shape(slide, Inches(0.7), card_top, Inches(11.933), evol_h, COLOR_SYNTHESIS_BG, COLOR_SYNTHESIS_BORDER)
        box_ev = slide.shapes.add_textbox(Inches(0.85), card_top + Inches(0.08), Inches(11.633), evol_h - Inches(0.16))
        tf_ev = box_ev.text_frame; tf_ev.word_wrap = True
        tf_ev.margin_left = Inches(0.08); tf_ev.margin_right = Inches(0.08)
        p_eh = tf_ev.paragraphs[0]
        p_eh.text = "🔄 STRATEGIC THINKING EVOLUTION & RESEARCH DISCOVERY"
        p_eh.font.size = Pt(10.5); p_eh.font.bold = True; p_eh.font.color.rgb = COLOR_PURPLE_PILL; p_eh.space_after = Pt(2)
        
        p_eb = tf_ev.add_paragraph()
        p_eb.font.size = Pt(9); p_eb.line_spacing = 1.15
        ...
        for p_idx, (q_txt, ans_txt) in enumerate(data["nlpPrompts"]):
            p_left = Inches(0.7) + p_idx * (nlp_w + nlp_gap)
            ...
            p = tf_p.paragraphs[0]
            p.text = "💬 " + q_txt
            p.font.size = Pt(9.5); p.font.bold = True; p.font.color.rgb = COLOR_MYNTRA_PINK; p.space_after = Pt(3)
            p_s = tf_p.add_paragraph()
            p_s.text = ans_txt
            p_s.font.size = Pt(8.8); p_s.line_spacing = 1.14; p_s.font.color.rgb = COLOR_TEXT_PRIMARY

        # Bottom: 3-Step NLP Pipeline & Testable Link
        pipe_top = nlp_top + nlp_h + Inches(0.12)
        pipe_h = card_height - (evol_h + nlp_h + Inches(0.24))
        add_card_shape(slide, Inches(0.7), pipe_top, Inches(11.933), pipe_h, COLOR_WHITE, COLOR_BORDER)
        box_pipe = slide.shapes.add_textbox(Inches(0.82), pipe_top + Inches(0.06), Inches(11.69), pipe_h - Inches(0.12))
        ...
        p.font.size = Pt(10); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(2)

        p_stages = tf_pipe.add_paragraph()
        p_stages.font.size = Pt(8.5); p_stages.line_spacing = 1.12
        ...
        p_link.font.size = Pt(8.5); p_link.space_before = Pt(2)
    ...
    # ==========================================
        ...
        positions = [
            (Inches(0.7), card_top),
            (Inches(6.76), card_top),
            (Inches(0.7), card_top + quad_h + Inches(0.18)),
            (Inches(6.76), card_top + quad_h + Inches(0.18))
        ]
        for q_idx, q_data in enumerate(data["quadrants"]):
            ...
            p.font.size = Pt(10.5); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(3)
            for b_lbl, b_txt in q_data["bullets"]:
                p = tf.add_paragraph()
                p.font.size = Pt(8.8); p.line_spacing = 1.15
    ...
    # ==========================================
        ...
        meth_h = Inches(0.48)
        add_card_shape(slide, Inches(0.7), card_top, Inches(11.933), meth_h, COLOR_SYNTHESIS_BG, COLOR_SYNTHESIS_BORDER)
        box_m = slide.shapes.add_textbox(Inches(0.85), card_top + Inches(0.04), Inches(11.633), meth_h - Inches(0.08))
        ...
        p_m.font.size = Pt(9.5); p_m.font.bold = True; p_m.font.color.rgb = COLOR_PURPLE_PILL

        # 2x3 Grid of 6 User Cards
        grid_top = card_top + meth_h + Inches(0.12)
        grid_w = Inches(3.86)
        grid_gap_x = Inches(0.17)
        ...
        for u_idx, user in enumerate(data["userCards"]):
            col = u_idx % 3
            row = u_idx // 3
            u_left = Inches(0.7) + col * (grid_w + grid_gap_x)
            ...
            p_uh.font.size = Pt(9.2); p_uh.font.bold = True; p_uh.font.color.rgb = COLOR_PURPLE_PILL; p_uh.space_after = Pt(2)
            
            # Authentic Quote Box
            p_q = tf_u.add_paragraph()
            p_q.font.size = Pt(8.4); p_q.line_spacing = 1.10; p_q.space_after = Pt(2)
            ...
            p_ins.font.size = Pt(8.4); p_ins.line_spacing = 1.10; p_ins.space_after = Pt(0)
    ...
    # ==========================================
        ...
        for q_idx in range(3):
            ...
            q_left = Inches(0.7) + q_idx * (q_w + q_gap)
            ...
            p.font.size = Pt(9.8); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(3)
            
            p_ans = tf.add_paragraph()
            p_ans.text = q_data["ans"]
            p_ans.font.size = Pt(8.6); p_ans.line_spacing = 1.14; p_ans.font.color.rgb = COLOR_TEXT_PRIMARY
        ...
        for q_idx in range(3, 5):
            q_data = data["pmQuestions"][q_idx]
            q_left = Inches(0.7) + (q_idx - 3) * (bot_w + bot_gap)
            ...
            if "customerValue" in q_data:
                ...
                p_c.font.size = Pt(8.6); p_c.line_spacing = 1.14; p_c.space_after = Pt(2)
                ...
                p_b.font.size = Pt(8.6); p_b.line_spacing = 1.14
    ...
    # ==========================================
        ...
        p_top = card_top
        p_h = Inches(1.35)
        add_card_shape(slide, Inches(0.7), p_top, Inches(11.933), p_h, COLOR_SYNTHESIS_BG, COLOR_SYNTHESIS_BORDER)
        box_p = slide.shapes.add_textbox(Inches(0.85), p_top + Inches(0.06), Inches(11.633), p_h - Inches(0.12))
        tf_p = box_p.text_frame; tf_p.word_wrap = True
        tf_p.margin_left = Inches(0.06); tf_p.margin_right = Inches(0.06)
        p = tf_p.paragraphs[0]
        ...
        for princ_title, princ_desc in data["principles"]:
            ...
            p_item.font.size = Pt(8.8); p_item.line_spacing = 1.12
        ...
        add_card_shape(slide, Inches(0.7), t_top, Inches(11.933), t_h, COLOR_BG_CARD, COLOR_BORDER)
        box_t = slide.shapes.add_textbox(Inches(0.82), t_top + Inches(0.08), Inches(11.69), t_h - Inches(0.16))
        ...
        for row in data["riceTable"]:
            ...
            p_row.font.size = Pt(8.8); p_row.line_spacing = 1.14
    ...
    # ==========================================
        ...
        pipe_h = Inches(1.55)
        add_card_shape(slide, Inches(0.7), card_top, Inches(11.933), pipe_h, COLOR_BG_CARD, COLOR_BORDER)
        box_p = slide.shapes.add_textbox(Inches(0.82), card_top + Inches(0.06), Inches(11.69), pipe_h - Inches(0.12))
        tf_p = box_p.text_frame; tf_p.word_wrap = True
        tf_p.margin_left = Inches(0.06); tf_p.margin_right = Inches(0.06)
        p = tf_p.paragraphs[0]
        ...
        p_arch.font.size = Pt(8.5); p_arch.line_spacing = 1.12
        ...
        for m_idx, feat in enumerate(data["mvpFeatures"]):
            m_left = Inches(0.7) + m_idx * (mvp_w + mvp_gap)
            ...
            p_cap.font.size = Pt(8.5); p_cap.line_spacing = 1.12; p_cap.space_after = Pt(2)
            ...
            if img_p and os.path.exists(img_p):
                slide.shapes.add_picture(img_p, m_left + Inches(0.2), mvp_top + Inches(1.1), mvp_w - Inches(0.4), mvp_h - Inches(1.2))
    ...
    # ==========================================
        ...
        left_w = Inches(6.8)
        add_card_shape(slide, Inches(0.7), card_top, left_w, card_height, COLOR_BG_CARD, COLOR_BORDER)
        box_m = slide.shapes.add_textbox(Inches(0.82), card_top + Inches(0.08), left_w - Inches(0.24), card_height - Inches(0.16))
        tf_m = box_m.text_frame; tf_m.word_wrap = True
        tf_m.margin_left = Inches(0.06); tf_m.margin_right = Inches(0.06)
        
        p = tf_m.paragraphs[0]
        p.text = "🎯 SUCCESS METRICS HIERARCHY (WITH SOURCED BASELINES)"
        p.font.size = Pt(10.5); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(4)

        for row in data["metricsTable"]:
            p_row = tf_m.add_paragraph()
            p_row.font.size = Pt(8.5); p_row.line_spacing = 1.14
            p_row.space_before = Pt(2); p_row.space_after = Pt(3)
            set_bullet_indent(p_row, left_margin_pt=14, hanging_indent_pt=0)
        ...
        for b_lbl, b_txt in data["experimentDesign"]:
            ...
            p_item.font.size = Pt(8.6); p_item.line_spacing = 1.14
    ...
    # ==========================================
        ...
        for p_idx, pit in enumerate(data["pitfalls"]):
            ...
            p = tf.paragraphs[0]
            p.text = "⚠️ " + pit["title"]
            p.font.size = Pt(9.8); p.font.bold = True; p.font.color.rgb = COLOR_MYNTRA_PINK; p.space_after = Pt(2)
            
            p_imp = tf.add_paragraph()
            p_imp.font.size = Pt(8.5); p_imp.line_spacing = 1.12; p_imp.space_after = Pt(2)
        ...
        for r_idx, phase in enumerate(data["rolloutPhases"]):
            ...
            p = tf.paragraphs[0]
            p.text = "🚀 " + phase["phase"]
            p.font.size = Pt(9.8); p.font.bold = True; p.font.color.rgb = COLOR_PURPLE_PILL; p.space_after = Pt(2)
